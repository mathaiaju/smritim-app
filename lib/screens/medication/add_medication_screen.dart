...
class _AddMedicationScreenState extends State<AddMedicationScreen> {
  ...
  Future<void> searchDrugs(String q) async {
    ...
    try {
      ...
      final List<String> drugs =
          data.map((d) => d['drug_name']).whereType<String>().toList();
      ...
      // 🔍 optional debug
      debugPrint('💊 drugSuggestions = $drugSuggestions');
    } catch (e) {
  ...
  Future<void> loadDrugSymptoms(String drug) async {
    try {
      // 🔍 for now, hardcode to English
      final lang = 'en';

      final res = await ApiClient.get(
        '/drugs/$drug/symptoms?lang=$lang',
      );

      final decoded = jsonDecode(res.body);

      final List<String> symptoms =
          List<String>.from(decoded['symptoms'] ?? const <String>[])
              .map((s) => s.replaceAll('/', '').trim()) // 🔥 FIX HERE
              .where((s) => s.isNotEmpty)
              .toList();

      setState(() {
        drugSymptoms = symptoms;
      });

      debugPrint('🩺 sanitized symptoms ($lang) → $drugSymptoms');
    } catch (e) {
      debugPrint('❌ symptom load failed for $drug: $e');
      setState(() {
        drugSymptoms = [];
      });
    }
